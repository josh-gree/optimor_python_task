...
try:
    browser = initialise_browser('./chromedriver')
...
assert [] not in extracted_countries, 'all selections not returned'
...
assert [] not in paygo_selections, 'all selections not returned'

paymonth_selections = [soup.select(paymonth_select) for soup in soups]
assert [] not in paymonth_selections, 'all selections not returned'
